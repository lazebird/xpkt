...
import { array2ipv6, array2num, checksum_calc, checksum_check, ipv6_change, num2hex, num_change } from './share';
...
function decode(arr: Array<number>, start: number) {
  const config: ProtocolNode = {
    ...
    children: [
      ...
      { key: 'sip', value: array2ipv6(arr.slice(start + 12, start + 16)), type: 'ipv6', pos: [start + 12, start + 15], change: (arr, e) => ipv6_change(arr, e.pos, e.value) },
      { key: 'dip', value: array2ipv6(arr.slice(start + 16, start + 20)), type: 'ipv6', pos: [start + 16, start + 19], change: (arr, e) => ipv6_change(arr, e.pos, e.value) },
    ],
  };
  return config;
}
